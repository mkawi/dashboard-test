...
(function(f) {
			...
			u = function(a) {
				if (c === p || c === k)
					(g = n[k]),
						...
						g && g._bAttrSrc
							? (Q(g.mData._)(d, l), w(g.mData.sort, a), w(g.mData.type, a), w(g.mData.filter, a))
							: m ? (g._setter || (g._setter = Q(g.mData)), g._setter(d, l)) : (d[k] = l);
				k++;
			};
		if (h)
			for (; h; ) {
	...
	function La(a, b, c, d) {
		...
		if (null === e.nTr) {
			...
			for (k = a.aoColumns.length; m < k; m++) {
				...
				if (
					l ||
					!((c && !w.mRender && w.mData === m) || (f.isPlainObject(w.mData) && w.mData._ === m + '.display'))
				)
	...
	function pb(a) {
		...
		for (b = k.length; l < b; l++) {
			...
			a.oFeatures.bSort &&
				...
				!1 !== n.bSortable &&
					(m.attr('tabindex', a.iTabIndex).attr('aria-controls', a.sTableId), Qa(a, n.nTh, l)));
			...
			if (0 !== l.length)
				...
				(c = h.sZeroRecords),
					...
					(b[0] = f('<tr/>', { class: e ? d[0] : '' }).append(
						f('<td />', { valign: 'top', colSpan: W(a), class: a.oClasses.sRowEmpty }).html(c)
					)[0]);
			...
			if ('<' == k) {
				...
				if ("'" == n || '"' == n) {
					...
					-1 != m.indexOf('.')
						...
						: '#' == m.charAt(0) ? (l.id = m.substr(1, m.length - 1)) : (l.className = m);
					u += p;
				}
				e.append(l);
				e = f(l);
	...
	function ub(a) {
		...
		h = function() {
			...
			b != e.sSearch &&
				(ia(a, { sSearch: b, bRegex: e.bRegex, bSmart: e.bSmart, bCaseInsensitive: e.bCaseInsensitive }),
				(a._iDisplayStart = 0),
				S(a));
		};
			...
			: 'number' === typeof b
				...
				: 'first' == b
					...
					: 'previous' == b
						...
						: 'next' == b
							? d + e < h && (d += e)
							: 'last' == b ? (d = Math.floor((h - 1) / e) * e) : O(a, 0, 'Unknown paging action: ' + b, 5);
	...
	function na(a) {
		...
		if (a.scrollBarVis !== D && a.scrollBarVis !== p) (a.scrollBarVis = D), aa(a);
			...
			'' === c
				? ((za.width = '100%'),
					x &&
						(r.find('tbody').height() > k.offsetHeight || 'scroll' == m.css('overflow-y')) &&
	...
	function Mb(a, b) {
		...
		for (var h, g = q.ext.type.order[c.sType + '-pre'], k = 0, f = a.aoData.length; k < f; k++)
			if (((c = a.aoData[k]), c._aSortData || (c._aSortData = []), !c._aSortData[b] || d))
		...
		for (d in b)
			if (b.hasOwnProperty(d)) {
				...
				f.isPlainObject(e)
					...
					: c && 'data' !== d && 'aaData' !== d && f.isArray(e) ? (a[d] = e.slice()) : (a[d] = e);
			}
		return a;
	}
	function $a(a, b, c) {
	...
	v.registerPlural = x = function(a, b, c) {
		...
		v.register(b, function() {
			...
			return a === this
				? this
				: a instanceof v ? (a.length ? (f.isArray(a[0]) ? new v(a.context, a[0]) : a[0]) : p) : a;
		});
	};
	var fc = function(a, b) {
		...
		gc = function(a, b, c) {
			...
			return eb(
				...
				function(b) {
					...
					if ('string' === typeof b && '#' === b.charAt(0) && ((e = a.aIds[b.replace(/^#/, '')]), e !== p))
						return [ e.idx ];
	...
	var hc = function(a, b, c, d) {
		...
		Yb = function(a, b) {
			...
			c.length &&
				a.length &&
				((a = c[0].aoData[a[0]]),
				a._details && ((a._detailsShow = b) ? a._details.insertAfter(a.nTr) : a._details.detach(), ic(c[0])));
		},
		ic = function(a) {
			var b = new v(a),
				c = a.aoData;
			...
			0 < J(c, '_details').length &&
				(b.on('draw.dt.DT_details', function(d, e) {
					a === e &&
						b.rows({ page: 'current' }).eq(0).each(function(a) {
							a = c[a];
	...
	t('row().child()', function(a, b) {
		...
		!0 === a
			? this.child.show()
			: !1 === a ? hb(this) : c.length && this.length && hc(c[0], c[0].aoData[this[0]], a, b);
		return this;
	});
				...
				if (null === b || b === p || c) {
					...
					for (q = e.length; m < q; m++)
						for (k = e[m], u = 0; u < l; u++)
							(t = { row: k, column: u }),
								c
									? ((v = d[k]), b(t, I(a, k, u), v.anCells ? v.anCells[u] : null) && n.push(t))
									: n.push(t);
					return n;
				}
